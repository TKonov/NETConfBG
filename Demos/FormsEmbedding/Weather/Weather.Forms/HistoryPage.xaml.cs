...
namespace Weather.Forms
{
    ...
    public partial class HistoryPage : ContentPage
    {
    	...
    	private void HistoryItemsOnItemTapped(object sender, ItemTappedEventArgs itemTappedEventArgs)
    	{
    		...
    		MessagingCenter.Send(this, HistoryItemSelected, historyItem.LocationName);
    	}
    }
	...
	public class HistoryRecorder
	{
		...
		{
			new HistoryItem(DateTime.Now.AddHours(-4), "9 C", "Seattle", "day200"),
			new HistoryItem(DateTime.Now.AddHours(-3), "18 C", "San Francisco", "day800"),
			new HistoryItem(DateTime.Now.AddHours(-2), "14 C", "Saint Louis", "day800"),
			new HistoryItem(DateTime.Now.AddHours(-1), "6 C", "Boston", "day800"),
			new HistoryItem(DateTime.Now.AddMinutes(-42), "15 C", "Denver", "day800"),
		};

        void AddHistory(HistoryItem historyItem)
        {
            var location = LocationHistory.FirstOrDefault(x => x.LocationName.ToLower() == historyItem.LocationName.ToLower());
            ...
            {
                location.Temperature = historyItem.Temperature;
            }
        }
	}
	...
	{
		internal HistoryItem(DateTime dateTime, string temp, string locationName, string icon)
		{
			DateTime = dateTime;
			Temperature = temp;
		...
		public HistoryItem(string temp, string locationName, string icon) : this(DateTime.Now, temp, locationName, icon)
		{
		}
